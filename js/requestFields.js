		...
		createRequest: function(){
			...
			for(var fieldXML in fieldsObj){
				if(fieldXML == "Fld__xml_ProductComponent"){
					...
					requestURL = requestURL + "&" + fieldXML + "=" + encodeURIComponent(prodCompObj2[fieldsObj[fieldXML]] || "")
				}else if(fieldXML == "Fld__xml_Release"){
					var releaseObj = JSON.parse(localStorage.getItem("releases"));
					requestURL = requestURL + "&" + fieldXML + "=" + encodeURIComponent(releaseObj[fieldsObj[fieldXML]] || "")
				}else if(fieldXML == "Fld__xml_NeedDate"){
					var dateArray = fieldsObj[fieldXML].split('-')[1] + "/"+ fieldsObj[fieldXML].split('-')[2] + "/"+ fieldsObj[fieldXML].split('-')[0];
