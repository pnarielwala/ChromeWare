      ...
      init: function () {
         ...
         if (self.options.bootstrap){
            self.$tool = $("<div id=\"\">"
			   ...
			   + "<button type=\"button\" id=\"saveaction\""
               + " title=\"Save screenshots\""
               + "class=\"btn btn-success " + classPosition2 + " annotate-save\">"
         ...
         if (self.options.position === "top" || (self.options.position !== "top" && !self.options.bootstrap)){
            self.$tool.css({"position": "absolute", "top": canvasPosition.top - 50, "width": "100%", "text-align": "center"});
         }else{
...
+ "padding:0px\"></textarea>");
		 ...
		 self.$tool.on("click", ".annotate-save", function(event){
            self.saveaction(event);
         });
	  ...
	  saveaction: function(event){
         ...
         self.redraw();
		 var self = this;
		 var dataURL = self.baseCanvas.toDataURL();
		 if(dataURL.length > 100){
			 var imageIndex = localStorage.getItem("editImageIndex");
			 var screenshots = JSON.parse(localStorage.Screenshots);
			 screenshots[imageIndex] = self.baseCanvas.toDataURL();
			 localStorage.setItem("Screenshots", JSON.stringify(screenshots));
			 chrome.tabs.query({currentWindow: true,active: true},function(tabs){chrome.tabs.remove(tabs[0].id);});
			 // new Alert("alert-success", "Saved!");
		 }else{
