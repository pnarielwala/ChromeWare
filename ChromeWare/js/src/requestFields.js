var _screenshotTool = require('./screenshot');
...
RequestFields.prototype.fillFields = function(){
	...
	$("input:checkbox").each(function(){
		var fieldId = $(this).attr('id');
		var fieldsObj = JSON.parse(localStorage.getItem("requestFields"));
		$(this).prop("checked", (fieldsObj[fieldId] == impactLayer.product));
	});
	this.fillImages();
};
...
RequestFields.prototype.initFieldEvents = function(){
	...
	$("#" + buttons.request).click(function(){
		if(localStorage.getItem("RequestCreationAllowed") == "false"){
			new Modal("danger", window.invalidMsgTitle, window.invalidSiteMsg).display();
		}else if(localStorage.getItem("LoggedOut") == "true"){
			new Modal("danger", window.invalidMsgTitle, "You are signed out of this site. Please log back in and try again").display();
		}else{
			myTransitions.createRequest();
			self.storeBuildData();
...
module.exports = new RequestFields();
